...
% ----------------------------------------------------------------------------------------------------
-module(imersia_companion_ssl_socket).
...
init(Req, _) ->
    ...
    %             true ->
    %                 Req0 = cowboy_req:set_resp_header(<<"sec-websocket-protocol">>, <<"mqtt">>, Req),
...
% TODO: Check the hidden status of the entity, and only allow events to be sent to hidden entities by the
% user who owns it.
...
websocket_info({message, _Ref, Message}, State) ->
    MessageToSend = case jsx:is_term(Message) of
        true -> Message;
        false -> <<"err">>
    end,
    {reply, {text, jsx:encode([{<<"message">>, MessageToSend}])}, State};
