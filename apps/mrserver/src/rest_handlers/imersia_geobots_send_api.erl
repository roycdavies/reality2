...
% Description: Send an event to a Geobot's automations
% ----------------------------------------------------------------------------------------------------
-module(imersia_geobots_send_api).
...
is_authorized(Req, State) ->
    ...
    case imersia_developer:check_and_log(DeveloperID, Location, cowboy_req:method(Req2), cowboy_req:uri(Req2), Parameters) of
        {ok, logged} ->
            ...
            Allowance = imersia_auth:check_geobot_attribute(Connection, post, GeobotID, UserID, ContextIDs, <<"send">>),
    ...
    case Event of
        ...
        _ ->
            ...
            case whereis(GeobotIDAtom) of
                undefined ->
                    imersia_db:close(Connection),
                    imersia_misc:response_error(geobotid, Req, State);
                _ ->
                    GeobotIDAtom ! {event, {Event, Parameters}},
                    imersia_misc:debug(debug, "Sending event ~s to ~s with parameters ~w. ~n", [Event, GeobotID, Parameters]),
                    imersia_db:close(Connection),
                    {true, cowboy_req:set_resp_body([<<"{\"ok\":\"sent\"}">>], Req), State}
            end
    end.
% ----------------------------------------------------------------------------------------------------
